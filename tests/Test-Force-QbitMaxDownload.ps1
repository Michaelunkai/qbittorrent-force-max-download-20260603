...
if ($content -match 'web_ui_password\s*=' -or $content -match 'web_ui_password_verify\s*=') { throw 'Script must not set qBittorrent WebUI passwords' }
if ($content -match 'bypass_local_auth\s*=\s*\$false') { throw 'Script must not disable local qBittorrent password bypass' }
